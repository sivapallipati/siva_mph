...
public class UserController {
 ...
 public ModelAndView memberHome() {
  ...
  model.addObject("memberName", user.getFirstname() + " " + user.getLastname());
  model.setViewName("member/memberHome");
  return model;
 }
 
 @RequestMapping(value= {"/provider/providerHome"}, method=RequestMethod.GET)
 public ModelAndView providerHome() {
  ModelAndView model = new ModelAndView();
  Authentication auth = SecurityContextHolder.getContext().getAuthentication();
  User user = userService.findUserByEmail(auth.getName());
  
  model.addObject("providerName", user.getFirstname() + " " + user.getLastname());
  model.setViewName("provider/providerHome");
  return model;
 }
